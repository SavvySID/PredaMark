...
const PredictionPageLinera = () => {
  ...
  const checkLineraConnection = async () => {
    try {
      lineraService.initialize();
      setLineraInitialized(true);
      // Try to get bet count to verify connection
      const count = await lineraService.getBetCount();
      setBetCount(count);
      setConnected(true);
    } catch (error) {
      console.error('Linera not connected:', error);
      setConnected(false);
      setLineraInitialized(false);
    }
  };

  const handleConnect = async () => {
    try {
      await checkLineraConnection();
      if (connected) {
        alert('✅ Connected to Linera!');
      } else {
        alert('❌ Failed to connect. Please check your Linera configuration.');
      }
    } catch (error) {
      console.error('Error connecting to Linera:', error);
      alert(error.message || 'Failed to connect to Linera.');
    }
  };
  ...
  const handleBet = async () => {
    ...
    setLoading(true);
    try {
      // Place bet using Linera service
      const prediction = selectedOutcome === 'yes';
      const success = await lineraService.placeBet(prediction);
      
      if (success) {
        // Update market prices (mock)
        setMarket(prev => ({
          ...prev,
          yesPrice: prediction ? prev.yesPrice + 0.01 : prev.yesPrice - 0.01,
          noPrice: !prediction ? prev.noPrice + 0.01 : prev.noPrice - 0.01,
        ...
        alert(`✅ Bet placed successfully!\n${selectedOutcome.toUpperCase()} for ${amount} units`);
        setAmount('');
      } else {
    ...
    <div className="prediction-page">
      <div className="market-header">
        <div className="market-info">
          ...
          <div className="linera-status" style={{ marginBottom: '20px', padding: '10px', background: connected ? '#d4edda' : '#f8d7da', borderRadius: '5px' }}>
            <strong>Linera Status:</strong> {connected ? '✅ Connected' : '❌ Not Connected'} | 
            Bet Count: {betCount}
          </div>
          ...
          <div className="bet-form">
            ...
            <div className="bet-summary">
              ...
              <div className="summary-row">
                ...
                <span>{amount ? parseFloat(amount).toFixed(2) : '0'} units</span>
              </div>
            </div>
            ...
            {recentBets.map(bet => (
              <div key={bet.id} className="recent-bet-item">
                <div className="bet-info">
                  ...
                  <span className="bet-amount">{bet.amount} units</span>
...
export default PredictionPageLinera;
